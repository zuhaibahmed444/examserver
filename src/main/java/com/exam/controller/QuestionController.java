...
//        return ResponseEntity.ok(list);
    ...
    public ResponseEntity<?> evalQuiz(@RequestBody List<Question> questions) {
        ...
        try {
        	String em = ((User)SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getEmail();
        	System.out.println(em);
        	SimpleMailMessage message = new SimpleMailMessage();
    		
    		message.setFrom("zuhaibahmed444@gmail.com");
    		message.setTo(em);
    		message.setText("REsults of the quiz taken as follows"+"\nMarksgot:"+marksGot+"\nCorrect Answers:" +correctAnswers+"\nQuestions attempted :" +attempted);
    		message.setSubject("Quiz Results");
    		
    		mailSender.send(message);
    		System.out.println("Mail of quiz");
